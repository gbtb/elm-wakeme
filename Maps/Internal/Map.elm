...
move offsetArg map =
    let
        centerOffset offset =
            Screen.Offset (map.width / 2 - offset.x) (map.height / 2 - offset.y)
    in
        { map | center = Screen.offsetToLatLng map <| centerOffset offsetArg }
...
viewBounds bounds map =
    { map | zoom = zoomHelper bounds map, center = Bounds.center bounds }


zoomHelper bounds map =
